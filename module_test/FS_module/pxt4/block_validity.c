 ...
 * inodes.
 */
static int add_system_zone(struct pxt4_sb_info *sbi,
			   pxt4_fsblk_t start_blk,
			   unsigned int count)
{
...
int pxt4_setup_system_zone(struct super_block *sb)
{
	...
	for (i=0; i < ngroups; i++) {
		if (pxt4_bg_has_super(sb, i) &&
		    ((i < 5) || ((i % flex_size) == 0)))
			add_system_zone(sbi, pxt4_group_first_block_no(sb, i),
					pxt4_bg_num_gdb(sb, i) + 1);
		gdp = pxt4_get_group_desc(sb, i, NULL);
		ret = add_system_zone(sbi, pxt4_block_bitmap(sb, gdp), 1);
		if (ret)
			return ret;
		ret = add_system_zone(sbi, pxt4_inode_bitmap(sb, gdp), 1);
		if (ret)
			return ret;
		ret = add_system_zone(sbi, pxt4_inode_table(sb, gdp),
				sbi->s_itb_per_group);
		if (ret)
			return ret;
	}
...
void pxt4_release_system_zone(struct super_block *sb)
{
	...
	rbtree_postorder_for_each_entry_safe(entry, n,
			&PXT4_SB(sb)->system_blks, node)
		kmem_cache_free(pxt4_system_zone_cachep, entry);

	PXT4_SB(sb)->system_blks = RB_ROOT;
}
