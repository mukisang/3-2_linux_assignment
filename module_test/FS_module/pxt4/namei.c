						...
						unsigned int line)
{
	...
	if (!is_dx_block) {
		if (pxt4_dirent_csum_verify(inode, dirent))
			set_buffer_verified(bh);
		else {
			pxt4_error_inode(inode, func, line, block,
...
void initialize_dirent_tail(struct pxt4_dir_entry_tail *t,
						   ...
						   struct pxt4_dir_entry *de)
{
	...
	d = de;
	top = (struct pxt4_dir_entry *)(((void *)de) +
...
#else
	t = PXT4_DIRENT_TAIL(de, PXT4_BLOCK_SIZE(inode->i_sb));
#endif
...
static __le32 pxt4_dirent_csum(struct inode *inode,
			       struct pxt4_dir_entry *dirent, int size)
{
...
int pxt4_dirent_csum_verify(struct inode *inode, struct pxt4_dir_entry *dirent)
{
	struct pxt4_dir_entry_tail *t;
	...
	if (t->det_checksum != pxt4_dirent_csum(inode, dirent,
						(void *)t - (void *)dirent))
		return 0;
...
static void pxt4_dirent_csum_set(struct inode *inode,
				 struct pxt4_dir_entry *dirent)
{
	struct pxt4_dir_entry_tail *t;
	...
	t->det_checksum = pxt4_dirent_csum(inode, dirent,
					   (void *)t - (void *)dirent);
}

int pxt4_handle_dirty_dirent_node(handle_t *handle,
				  struct inode *inode,
				  struct buffer_head *bh)
{
	pxt4_dirent_csum_set(inode, (struct pxt4_dir_entry *)bh->b_data);
		...
		if (de->inode)
		{
			if (show_names)
			{
				...
				if (!fscrypt_has_encryption_key(dir)) {
					...
					pxt4fs_dirhash(de->name, de->name_len,
						       &h);
				...
				int len = de->name_len;
				char *name = de->name;
				pxt4fs_dirhash(de->name, de->name_len, &h);
	 ...
	 struct dx_hash_info *hinfo, struct dx_frame *frame_in)
{
	...
	if (fname && fname_name(fname))
		pxt4fs_dirhash(fname_name(fname), fname_len(fname), hinfo);
	hash = hinfo->hash;
				  ...
				  __u32 start_hash, __u32 start_minor_hash)
{
	...
	for (; de < top; de = pxt4_next_entry(de, dir->i_sb->s_blocksize)) {
		...
		pxt4fs_dirhash(de->name, de->name_len, hinfo);
 ...
 */
static inline bool pxt4_match(const struct pxt4_filename *fname,
					...
					int *inlined)
{
	...
	*res_dir = NULL;
	...
	retval = pxt4_fname_setup_filename(dir, d_name, 1, &fname);
	if (retval == -ENOENT)
		return NULL;
	if (retval)
			...
			struct dx_hash_info *hinfo)
{
	...
	de2->rec_len = pxt4_rec_len_to_disk(data2 + (blocksize - csum_size) -
					    ...
					    blocksize);
	if (csum_size) {
		t = PXT4_DIRENT_TAIL(data2, blocksize);
		initialize_dirent_tail(t, blocksize);

		t = PXT4_DIRENT_TAIL(data1, blocksize);
		initialize_dirent_tail(t, blocksize);
	}
	...
	err = pxt4_handle_dirty_dirent_node(handle, dir, bh2);
	if (err)
		goto journal_error;
			     ...
			     struct buffer_head *bh)
{
	...
	err = pxt4_handle_dirty_dirent_node(handle, dir, bh);
	if (err)
		pxt4_std_error(dir->i_sb, err);
	return 0;
}
			    ...
			    struct inode *inode, struct buffer_head *bh)
{
	...
	memcpy (data1, de, len);
	de = (struct pxt4_dir_entry_2 *) data1;
	top = data1 + len;
	...
	pxt4fs_dirhash(fname_name(fname), fname_len(fname), &fname->hinfo);
	...
	if (retval)
		goto out_frames;	
	retval = pxt4_handle_dirty_dirent_node(handle, dir, bh2);
	if (retval)
		goto out_frames;	
			  ...
			  struct inode *inode)
{
	...
	if (csum_size) {
		t = PXT4_DIRENT_TAIL(bh->b_data, blocksize);
		initialize_dirent_tail(t, blocksize);
	}
...
again:
	...
	at = frame->at;
	bh = pxt4_read_dirblock(dir, dx_get_block(frame->at), DIRENT);
	if (IS_ERR(bh)) {
		err = PTR_ERR(bh);
		bh = NULL;
		goto cleanup;
			     ...
			     struct inode *inode)
{
	...
	set_nlink(inode, 2);
	if (csum_size) {
		t = PXT4_DIRENT_TAIL(dir_block->b_data, blocksize);
		initialize_dirent_tail(t, blocksize);
	}

	BUFFER_TRACE(dir_block, "call pxt4_handle_dirty_metadata");
	err = pxt4_handle_dirty_dirent_node(handle, inode, dir_block);
	if (err)
		goto out;
...
bool pxt4_empty_dir(struct inode *inode)
{
	...
	bh = pxt4_read_dirblock(inode, 0, EITHER);
	if (IS_ERR(bh))
		return true;
				  ...
				  unsigned dir_ino)
{
	...
	if (!ent->dir_inlined) {
		if (is_dx(ent->inode)) {
			...
			retval = pxt4_handle_dirty_dirent_node(handle,
							       ent->inode,
							       ent->dir_bh);
		}
	} else {
