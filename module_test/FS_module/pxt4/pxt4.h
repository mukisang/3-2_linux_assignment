...
#define PXT4_FL_USER_VISIBLE		0x304BDFFF /* User visible flags */
#define PXT4_FL_USER_MODIFIABLE		0x204BC0FF /* User modifiable flags */
...
#define PXT4_REG_FLMASK (~(PXT4_DIRSYNC_FL | PXT4_TOPDIR_FL))
					  ...
					  __le32 extra)
{
	if (unlikely(extra & cpu_to_le32(PXT4_EPOCH_MASK))) {
		 ...
		 * at the discretion of the pxt4 developers.)
		 */
		u64 extra_bits = le32_to_cpu(extra) & PXT4_EPOCH_MASK;
			...
			int lookup, struct pxt4_filename *fname)
{
	...
	memset(fname, 0, sizeof(struct pxt4_filename));
...
/* hash.c */
extern int pxt4fs_dirhash(const char *name, int len, struct
			  dx_hash_info *hinfo);
...
/* namei.c */
extern int pxt4_dirent_csum_verify(struct inode *inode,
...
static inline pxt4_group_t pxt4_flex_group(struct pxt4_sb_info *sbi,
					     pxt4_group_t block_group)
{
	printk("s_log_groups_per_flex : %d\n",sbi->s_log_groups_per_flex);//printKyle
...
extern void initialize_dirent_tail(struct pxt4_dir_entry_tail *t,
				   unsigned int blocksize);
extern int pxt4_handle_dirty_dirent_node(handle_t *handle,
					 struct inode *inode,
					 struct buffer_head *bh);
#define S_SHIFT 12
