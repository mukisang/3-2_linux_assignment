...
/*
 * pxt4_es_remove_extent() removes a space from a extent status tree.
 *
 * Return 0 on success, error code on failure.
 */
...
/*
 * pxt4_es_remove_blks - remove block range from extents status tree and
 ...
 *
 */
void pxt4_es_remove_blks(struct inode *inode, pxt4_lblk_t lblk,
			 pxt4_lblk_t len)
{
	struct pxt4_sb_info *sbi = PXT4_SB(inode->i_sb);
	unsigned int clu_size, reserved = 0;
	pxt4_lblk_t last_lclu, first, length, remainder, last;
	bool delonly;
	int err = 0;
	struct pending_reservation *pr;
	struct pxt4_pending_tree *tree;
	...
	last_lclu = PXT4_B2C(sbi, lblk + len - 1);
	...
	for (first = lblk, remainder = len;
		...
		if (PXT4_B2C(sbi, first) == last_lclu)
			length = remainder;
		else
			length = clu_size - PXT4_LBLK_COFF(sbi, first);
		 ...
		 * delayed only status.
		 */
		delonly = __es_scan_clu(inode, &pxt4_es_is_delonly, first);
		 ...
		 */
		if (delonly &&
		    !__es_scan_clu(inode, &pxt4_es_is_delonly, first)) {
			pr = __get_pending(inode, PXT4_B2C(sbi, first));
			if (pr != NULL) {
				tree = &PXT4_I(inode)->i_pending_tree;
				rb_erase(&pr->rb_node, &tree->root);
				kmem_cache_free(pxt4_pending_cachep, pr);
