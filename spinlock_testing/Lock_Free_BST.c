...
vector rlist[MAX_THREADS];//c
int hp_off[MAX_THREADS];
pthread_mutex_t hp_lock = PTHREAD_MUTEX_INITIALIZER,hp_lock2=PTHREAD_MUTEX_INITIALIZER;
...
void LF_add(int key, int thread_num)//c
{
	...
	while (true) {
		 ...
		 */
		result = LF_find(key, &pred, &pred_op, &curr, &curr_op, base_root, thread_num);//c
		if (hazard_pointers) {
		 ...
		 */
		cas_op = malloc(sizeof(Child_CAS_OP));//c
...
//	printf("[%d,%d] STEP FINAL | out to the LF_add\n",thread_num,key);
}
// sang muk is the legend
int LF_find(int key, LF_BST_Node **pred, void **pred_op, LF_BST_Node **curr, void **curr_op, LF_BST_Node *auxRoot, int thread_num)//c
{
	int result, curr_key;
...
bool LF_remove(int key, int thread_num)//c
{
	...
	while(true) {
		 ...
		 */
		if(LF_find(key, &pred, &pred_op, &curr, &curr_op, base_root, thread_num) != FOUND) {//c
			return false;
		}
		...
		if( IS_NULL(curr->right) || IS_NULL(curr->left) ) {
			 ...
			 */
			if( (LF_find(key, &pred, &pred_op, &replace, &replace_op, curr, thread_num) == ABORT) || (curr->op != curr_op) ) {//c
				continue;
			}
			 ...
			 */
			Relocate_OP* reloc_op = (Relocate_OP* )malloc(sizeof(Relocate_OP));//c
...
void add_to_hp_list(int thread_num, LF_BST_Node *node)
{
	...
	{
		return ;
		pthread_mutex_lock(&hp_lock);//c
		hp[hp_off[thread_num]] = SET_NULL(node);
		pthread_mutex_unlock(&hp_lock);//c
	}
	else
	{
		pthread_mutex_lock(&hp_lock);//c
...
void helpChildCAS(Child_CAS_OP *op, LF_BST_Node *dest, int thread_num)
{
	...
	if (__sync_bool_compare_and_swap(address, op->expected, op->update) && hazard_pointers) {
		if (UNFLAG(op->expected) != NULL) {
			...
			*/
			vector_find(&rlist[thread_num],(LF_BST_Node *)UNFLAG(op->expected));
			if(rlist[thread_num].count==0)
				vector_push_back(&rlist[thread_num],(LF_BST_Node *)UNFLAG(op->expected));//c
			else if(rlist[thread_num].count!=0&&vector_find(&rlist[thread_num],(LF_BST_Node *)UNFLAG(op->expected))==rlist[thread_num].data[(rlist[thread_num].count)-1])
			{
				vector_push_back(&rlist[thread_num],(LF_BST_Node *)UNFLAG(op->expected));//c
			}
		...
		if (rlist[thread_num].count > HP_THRESHOLD) {//c
			/*
			...
			*/
			printf("pleaseeeeeeeeeeeeeeeeeeeeeeeeeeeeeee\n");
			vector temp_rlist=rlist[thread_num];
			for(int i=0;i<temp_rlist.count;i++)
			{
				LF_BST_Node *retired_node = temp_rlist.data[i];
				bool ok_to_free=true;

				for(int i=0;i<MAX_THREADS * NUM_HP_PER_THREAD;i++) {
					pthread_mutex_lock(&hp_lock2);//c
					LF_BST_Node *tmp = UNFLAG(hp[i]);
					pthread_mutex_unlock(&hp_lock2);
					if(retired_node == tmp){
						ok_to_free=false;
						break;
					}
				...
				if(ok_to_free) {
					free(retired_node);//c
					vector_erase(&rlist[thread_num],vector_find(&rlist[thread_num],retired_node));
					}//c
...
void helpMarked(LF_BST_Node *pred, void *pred_op, LF_BST_Node *curr, int thread_num)
{
	...
	cas_op = (Child_CAS_OP* )malloc(sizeof(Child_CAS_OP));//c
...
{
	LF_BST_Node *newNode = (LF_BST_Node*)malloc(sizeof(LF_BST_Node));//c
	newNode->key = key;
	//newNode->op = SET_NULL(NULL);
	newNode->op = NULL;
