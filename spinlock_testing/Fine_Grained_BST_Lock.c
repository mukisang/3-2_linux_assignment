...
void FG_spin_search(int val, FG_BST_Node *root, FG_BST_Node *parent)
{
	...
	if(val < root->value) {
		if (root->left == NULL) {
			...
			pthread_spin_lock(&root->left->lock);
			pthread_spin_unlock(&root->lock);
			FG_spin_search(val, root->left, root);
		}
	}
		...
		if (root->right == NULL) {
			...
			pthread_spin_lock(&root->right->lock);
			pthread_spin_unlock(&root->lock);
			FG_spin_search(val, root->right, root);
		}
	} else {
		pthread_spin_unlock(&root->lock);
	}
...
void FG_spin_insert(int val, FG_BST_Node* root, FG_BST_Node *parent, int thread_num) {
	...
	if(parent == NULL) { //I am at the root
		pthread_spin_lock(&tree_lock);
		if(g_root == NULL) {
			g_root = spin_createNode(val, parent);
			pthread_spin_unlock(&tree_lock);
			return;
		}
		pthread_spin_lock(&g_root->lock);
	...
	if(val < root->value) {
		if (root->left == NULL) {
			root->left = spin_createNode(val, root);
			pthread_spin_unlock(&root->lock);
		} else {
			pthread_spin_lock(&root->left->lock);
			pthread_spin_unlock(&root->lock);
			FG_spin_insert(val, root->left, root, thread_num);
		}
	}
	else if (val > root->value) {
		if (root->right == NULL) {
			root->right = spin_createNode(val, root);
			pthread_spin_unlock(&root->lock);
		} else {
			pthread_spin_lock(&root->right->lock);
			pthread_spin_unlock(&root->lock);
			FG_spin_insert(val, root->right, root, thread_num);
		}
	} else {
	...
	if(val == root->value) {
		...
		if (root->right == NULL) {
			...
			if (val == root->right->value) {
				...
				pthread_spin_unlock(&root->lock);
				return spin_del_search(val, root->right, thread_num);
			}
		}
	}
...
int FG_spin_remove(int val, FG_BST_Node *root, int thread_num)
{
	...
	to_be_deleted = spin_del_search(val, root, thread_num);
	...
	if (to_be_deleted->right != NULL) {
		...
		pthread_spin_lock(&to_be_deleted->right->lock);
		if (to_be_deleted->right->left == NULL) {
			...
			if (successor->right != NULL) {
				pthread_spin_lock(&successor->right->lock);
				to_be_deleted->value = successor->value;
				to_be_deleted->right = successor->right;
				successor->right->parent = to_be_deleted;
				pthread_spin_unlock(&successor->right->lock);
			} else {
				to_be_deleted->value = successor->value;
				to_be_deleted->right = NULL;
			}
			//pthread_spin_unlock(&successor->lock);
			free(successor);
			pthread_spin_unlock(&to_be_deleted->lock);
			return 0;
		}
		...
		successor = spin_get_inorder_successor(to_be_deleted);
		...
		free(successor);
		pthread_spin_unlock(&successor_parent->lock);
		pthread_spin_unlock(&to_be_deleted->lock);
		return 0;
	}
	...
	if (to_be_deleted->left != NULL) {
		pthread_spin_lock(&to_be_deleted->left->lock);
		// perform pre-emptive check
		...
		predecessor = spin_get_inorder_predecessor(to_be_deleted);
		...
		free(predecessor);
		pthread_spin_unlock(&predecessor_parent->lock);
		pthread_spin_unlock(&to_be_deleted->lock);
		return 0;
	}
 ...
 */
FG_BST_Node *spin_get_inorder_successor(FG_BST_Node *node)
{
	...
	// lock the successor
	pthread_spin_lock(&successor->lock);
 ...
 */
FG_BST_Node *spin_get_inorder_predecessor(FG_BST_Node *node)
{
	...
	pthread_spin_lock(&predecessor->lock);
